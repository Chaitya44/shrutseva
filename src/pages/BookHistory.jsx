  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        const res = await fetch(`/front/book_history?bhandar_code=${encodeURIComponent(bhandar)}`);
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="text-[28px] font-heading font-extrabold tracking-tight">
            <span className="text-[#012c77]">Book</span><span className="text-[#FF6B00]"> History</span>
          </h1>
          <span className="text-[11px] font-bold text-neutral-400 uppercase tracking-wide">Complete book transaction audit trail</span>
        </div>
        <BhandarPageSelector />
      </div>
