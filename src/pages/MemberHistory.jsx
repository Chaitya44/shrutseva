  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        const res = await fetch(`/front/member_history?bhandar_code=${encodeURIComponent(bhandar)}`);
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="text-[28px] font-heading font-extrabold tracking-tight">
            <span className="text-[#012c77]">Member</span><span className="text-[#FF6B00]"> History</span>
          </h1>
          <span className="text-[11px] font-bold text-neutral-400 uppercase tracking-wide">Member activity and book transaction records</span>
        </div>
        <BhandarPageSelector />
      </div>
            ...
            <tbody>
              {data.map((row, i) => (
                <tr key={i} className={`border-b border-neutral-50 hover:bg-blue-50/50 transition-colors ${i%2===0?'bg-white':'bg-neutral-50/40'}`}>
                  <td className="px-5 py-3.5 font-semibold text-neutral-700">{row.member_name||'-'}</td>
                  <td className="px-5 py-3.5 text-neutral-500">{row.book_name||'-'}</td>
                  <td className="px-5 py-3.5">
                    <span className="px-2 py-0.5 rounded-full text-[11px] font-bold bg-teal-50 text-teal-700 border border-teal-100">
                      {row.action||'-'}
                    </span>
                  </td>
