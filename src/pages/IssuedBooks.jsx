  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        const res = await fetch(`/front/quick_advance_book_search?issued=1&bhandar=${encodeURIComponent(bhandar)}`);
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="text-[28px] font-heading font-extrabold tracking-tight">
            <span className="text-[#012c77]">Issued</span><span className="text-[#FF6B00]">Books</span>
          </h1>
          <span className="text-[11px] font-bold text-neutral-400 uppercase tracking-wide">Currently issued book records</span>
        </div>
        <BhandarPageSelector />
      </div>
