...
export default function CreatedBooks() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        const res = await fetch(`/front/createdbooks?bhandar_code=${encodeURIComponent(bhandar)}`);
    ...
    <div className="w-full max-w-[1400px] px-4 sm:px-6 mx-auto pb-8 pt-14 sm:pt-16">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="text-[28px] font-heading font-extrabold tracking-tight">
            <span className="text-[#012c77]">Created</span><span className="text-[#FF6B00]"> Books</span>
          </h1>
          <span className="text-[11px] font-bold text-neutral-400 uppercase tracking-wide">Recently added books to the catalog</span>
        </div>
        <BhandarPageSelector />
      </div>
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center h-48 text-neutral-400">
          <FilePlus2 size={40} className="mb-3 opacity-30" />
          ...
          <table className="w-full text-[13px]">
            <thead>
              <tr className="bg-gradient-to-r from-[#012c77] to-[#1D4ED8] text-white">
                ...
                <th className="px-5 py-3 font-bold text-left">Created At</th>
              </tr>
            </thead>
            <tbody>
              {data.map((row, i) => (
